  ...
  Future<void>shareReport()async{
    setState(()=>busy=true);try{
      ...
      final pdf=pw.Document();pdf.addPage(pw.MultiPage(pageFormat:PdfPageFormat.letter,build:(_)=>[
        ...
        pw.SizedBox(height:8),pw.Text('Agente: ${s['agentName']}'),pw.Text('Telefono: ${s['phone']}'),pw.Text('Fecha: ${today()}'),pw.Text('Regla remesas: por debajo de ${money(numv(s['remittanceThreshold']))} = ${money(numv(s['remittanceFlatFee']))} fijo · Agente ${numv(s['remittanceAgentSharePct']).toStringAsFixed(1)}%'),
